...
function addToGroup(drinkId, drinkName){
	const drinkItem = cartGroup.find(item => item.idDrink == drinkId);
	if(drinkItem){
		...
		cartGroup.push({idDrink:drinkId, name: drinkName}); // ---
		showDrinkGroup();
	}
}

function showDrinkDetails(drinkId){
	// search full cocktail details by id // eg. www.thecocktaildb.com/api/json/v1/1/lookup.php?i=11007
...
function showModal(drink){
	//console.log("drink detail", drink.strDrink, drink["strDrink"]);
	let modalBody = document.getElementById("modal-body");
	modalBody.innerHTML = `
		<!-- <img src="${drink.strImageSource}" alt="drink-pic"> problem -->
...
function showDrinkGroup(){
	...
	cartGroup.forEach(e=>{
		let div = document.createElement("div");
		div.classList+=("card mb-1 p-1");
		div.innerHTML= `
			<p>name: ${e.name}</p>
			<p>id: ${e.idDrink}</p>
		`;
		groupList.appendChild(div);
		groupListCount.innerHTML = `<h3>Group List: (${cartGroup.length})</h3>`;
	});	
}
